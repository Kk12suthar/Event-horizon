...
export function TopBar({ breadcrumb, pageTitle, user, onLogout, onNavigate, onMobileMenuToggle, notifications }: TopBarProps) {
  ...
  return (
    <header className="flex items-center justify-between h-14 px-4 lg:px-6 bg-[#000000] border-b border-[#262626] flex-shrink-0">
      {/* Left */}
      <div className="flex items-center gap-3 min-w-0">
        <button
          onClick={onMobileMenuToggle}
          className="lg:hidden w-9 h-9 flex items-center justify-center rounded-lg text-[#B8B8B8] hover:text-white hover:bg-[#181818] transition-colors"
        >
      ...
      <div className="flex items-center gap-2">
        {/* Notifications */}
        <button className="relative w-9 h-9 flex items-center justify-center rounded-lg text-[#B8B8B8] hover:text-white hover:bg-[#181818] transition-colors">
        ...
        <DropdownMenu open={showUserMenu} onOpenChange={setShowUserMenu}>
          <DropdownMenuTrigger asChild>
            <button className="flex items-center gap-2 ml-2 rounded-lg hover:bg-[#181818] transition-colors px-2 py-1.5">
          ...
          <DropdownMenuContent
            align="end"
            className="w-[200px] bg-[#0D0D0D] border-[#262626] text-white"
          >
            <div className="px-3 py-2">
              <p className="text-sm font-semibold">{user?.name}</p>
              <p className="text-xs text-[#B8B8B8]">{user?.email}</p>
              <span className="inline-block mt-1.5 px-2 py-0.5 bg-white/[0.06] text-[#E4E4E7] text-[10px] font-medium uppercase tracking-wider rounded-full">
                {user?.role}
              </span>
            </div>
