...
import { ArrowRight, CornerDownRight, Radio, Terminal } from "lucide-react";

interface ClosingCTAProps {
  /** Optional handler invoked after the submit animation (routes into sign up). */
  onLaunch?: () => void;
}

export const ClosingCTA: React.FC<ClosingCTAProps> = ({ onLaunch }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    // Trigger cinematic warp transition, then hand off to sign up.
    setIsWarping(true);
    setTimeout(() => {
      setIsWarping(false);
      if (onLaunch) {
  ...
  return (
    ...
    >
      ...
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[300px] h-[300px] rounded-full bg-black border border-white/[0.04] shadow-[0_0_120px_rgba(193, 110, 67,0.1)] flex items-center justify-center pointer-events-none">
      ...
      <div className="w-full max-w-3xl text-center space-y-12 relative z-10">
        ...
        <div className="space-y-4">
          ...
          <motion.p
            ...
            id="cta-description"
          >
            Create a project, bring your data, and let the agent prepare, visualize,
            and publish it. Your workspace is one click away.
          </motion.p>
        </div>
          ...
          {!submitted ? (
            ...
            >
              ...
              <div className="mt-3 flex items-center justify-center gap-2 text-[9px] font-mono text-zinc-600">
                ...
                <span>TAKES YOU STRAIGHT TO SIGN UP</span>
              </div>
            </motion.form>
          ) : (
        ...
        <div className="pt-24 border-t border-white/[0.03] flex flex-col md:flex-row justify-between items-center gap-4 text-[10px] font-mono text-zinc-600">
          ...
          <div className="flex gap-6">
            <a href="#section-pull" className="hover:text-zinc-400 transition-colors">SOURCES</a>
            <a href="#section-transform" className="hover:text-zinc-400 transition-colors">PREPARE</a>
            <a href="#section-capabilities" className="hover:text-zinc-400 transition-colors">WORKSPACE</a>
          </div>
        </div>
      </div>
    </section>
