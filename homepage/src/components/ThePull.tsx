...
import { Activity, ArrowDown, HelpCircle, Magnet, Orbit, Radio } from "lucide-react";
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < 160; i++) {
      points.push({
        ...
        color: Math.random() > 0.6 ? "rgba(255, 92, 0, " : "rgba(139, 92, 246, ",
    ...
    const draw = () => {
      ...
      ctx.fillStyle = "rgba(255, 92, 0, 0.03)";
      ctx.beginPath();
      ...
      points.forEach((p) => {
        ...
        points.forEach((p2) => {
          ...
          if (cdist < 1400) {
            ...
            ctx.strokeStyle = `rgba(255, 92, 0, ${connAlpha.toFixed(2)})`;
            ctx.lineWidth = 0.5;
            ctx.beginPath();
            ctx.moveTo(p.x, p.y);
    ...
    >
      <div className="w-full max-w-7xl grid grid-cols-1 lg:grid-cols-12 gap-12 items-center">
        ...
        <div className="lg:col-span-5 space-y-8 text-left">
          <div className="inline-flex items-center gap-2 px-3 py-1 bg-white/[0.01] border border-white/[0.05] rounded-full">
            <Orbit className="w-3.5 h-3.5 text-plasma-purple" />
            <span className="font-mono text-[9px] tracking-widest text-zinc-400 uppercase">
              Phase 01 :: Intake & Attractor
            </span>
          </div>
          ...
          <h2 className="font-display font-semibold text-3xl md:text-5xl text-white tracking-tight uppercase">
            ...
            <span className="block text-zinc-500 font-light text-xl md:text-2xl mt-2 lowercase font-sans">
              capturing chaos at lightspeed
            </span>
          </h2>

          <p className="text-zinc-400 text-sm font-sans leading-relaxed font-light">
            No schemas, no structures, no sorting. Every cluster of data is drawn
            into the event horizon by a massive lensed gravity matrix. Our intake pipeline
            can absorb structured logs, unstructured raw payloads, and chaotic JSON grids,
            pulling them into orbit where they are pre-aligned for quantum compression.
          </p>
          ...
          <div className="p-4 bg-white/[0.01] border border-white/[0.04] rounded-lg flex items-start gap-4">
            ...
            <div className="space-y-1">
              <h4 className="text-xs font-mono text-white tracking-wide uppercase">
                CURSOR GRAVITY ATTRACTION
              </h4>
              <p className="text-[11px] text-zinc-500 leading-normal">
                Move your cursor over the telemetry intake grid on the right to simulate 
                localized high-mass gravity. Watch coordinates bend and orbit.
              </p>
            </div>
          </div>
